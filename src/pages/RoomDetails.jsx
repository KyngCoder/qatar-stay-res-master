...
const RoomsDetails = () => {
  ...
  const center = useMemo(() => [51.505, -0.091], []);
...
const amenities = selectedRoom.amenities?.map( (amenity, index) => {
    return (
        <div
        className={`shadow-lg rounded-full items-center flex justify-center py-2 px-4 gap-2  ${amenity.type === "free" ? 'bg-green-300' : 'bg-red-300'}`}
        key={index}
    >
        <img src={amenity.icon} height="full" className="relative translate-y-2" width="20px" />
  ...
  return (
    ...
    <div> 
        <table className='table-auto'>
            ...
            <tr>
                <td><b>Rating</b></td>
                <td> <FaStar className='text-blue-700 inline' /> {selectedRoom.ratings}</td>
            </tr>
      ...
      <div onClick={()=>reserve(selectedRoom)} to="/book" className="flex  w-full justify-center">
